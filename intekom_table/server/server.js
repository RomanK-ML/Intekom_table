...
class DataBaseServer {
  ...
  constructor() {
    ...
    for (let i = 1; i <= 120; i++) {
      this.CallsDb2.push({
        ...
        phoneNumber: "+7(999) 666-" + (4000 + i),
        contact_name: "Контакт_ " + i,
        expectation: parseInt(i%6 + 1),
        duration: parseInt(i%18 + 1),
        type_call: (i % 2 === 0 ? "Входящий" : "Исходящий"),
        status: (i % 3 === 0 ? "Ответ" : "Нет ответа")
      });
    }
  ...
  createRoutes() {
    ...
    this.app.all("/api/data2", async (req, res) => {
      console.log("DATA")
      try {
        req.params
        let { page, size  } = req.query;
        ...
        const last_page = Math.ceil(this.CallsDb2.length / size );

        const calls = this.callList2(page, size);
        const dataLabels2 = [
          { title: "№", field: "id", headerFilter: true },
          { title: "Дата", field: "date", headerFilter: true, editor: "input", validator: "minLength:10", editorParams: { search: true } },
          { title: "Номер", field: "phoneNumber", editor: "input", headerFilter: true, validator: "minLength:16", editorParams: { search: true } },
        ...
        res.json({ last_page: last_page, dataLabels: dataLabels2, data: calls });
      } catch (error) {
        console.error(error);
        res
            .status(500)
  ...
  callList2(page, size){
    const startIndex = parseInt((page -  1) * size);
    const endIndex = parseInt(startIndex + size);
    return this.CallsDb2.slice(startIndex, endIndex);
  }
